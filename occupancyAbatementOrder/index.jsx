...
const OccupancyAbatementOrder = () => {

  const [abatementOrder, setAbatementOrder] = useState([]);

  const theme = useTheme();
  const colors = tokens(theme.palette.mode);

  const columns = [
    { field: "ao_id", headerName: "No.", width: 50 },
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8080/abatementorder/getAllAbatementOrder")
      .then((res) => res.json()) // Parse the response as JSON
      .then((result) => {
        
        setAbatementOrder(result);
      })
      .catch((error) => {
...
/*
  ...
  return (
    <Box m="15px">
      <Header title="FSIC -> Occupancy -> Abatement Order" />
      <Box sx={{ 
        width: '30%', 
        mt: -2,
...
export default OccupancyAbatementOrder;